...
const amountRaisedFormatted = totalAmountRaised.toLocaleString(); // Adds commas to the number
const unfundedGamesString = `A total of $${amountRaisedFormatted} has been raised for ${totalNumberOfGames} games. Currently, ${numberOfUnfundedGames} ${numberOfUnfundedGames === 1 ? "game remains" : "games remain"} unfunded.`;
...
topGameElement.innerHTML = `<h2>${topGame.name}</h2><p>Raised: $${topGame.pledged.toLocaleString()}</p>`;
firstGameContainer.appendChild(topGameElement);

// do the same for the runner up item
const runnerUpGameElement = document.createElement("div");
runnerUpGameElement.innerHTML = `<h2>${runnerUpGame.name}</h2><p>Raised: $${runnerUpGame.pledged.toLocaleString()}</p>`;
